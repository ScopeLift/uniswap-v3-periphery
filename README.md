...
**This branch is configured to facilitate testing gas usage of calls to the `getPopulatedTicksInWord()` method**
...
To prepare for running the OVM tests:
1. Run `yarn optimism-up` to clone the optimism repo and switch to the correct branch
2. Open `geth.env` and set `TARGET_GAS_LIMIT=100000000`. This is to ensure calls do not revert

And to run them:
1. Open `PoolAddress.sol` and set `POOL_INIT_CODE_HASH` to `0x789bbcb4d36a7a15d68e07c192ff256236e3486610523115567871e609a917ca`
2. Run tests with `yarn optimism-down && yarn optimism-up && UPDATE_SNAPSHOT=1 yarn test:ovm`. This is required so the deployer account nonce is reset to zero, which is necessary to get the above bytecode hash when deploying contracts

After running the EVM or OVM tests, you can view the gas cost of the `getPopulatedTicksInWord` method by viewing `test/__snapshots__/TickLens.spec.ts.snap`.
